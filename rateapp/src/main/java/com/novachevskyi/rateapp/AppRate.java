package com.novachevskyi.rateapp;
...
public class AppRate implements OnClickListener, OnCancelListener {
  ...
  /**
   * @param minLaunchesUntilPrompt The minimum number of times the user lunches the application
   * before showing the rate dialog.<br/> Default value is 0 times.
  ...
  /**
   * @param showIfCrash If <code>false</code> the rate dialog will not be shown if the application
   * has crashed once.<br/> Default value is <code>false</code>.
   * @return This {@link AppRate} object to allow chaining.
   */
  ...
  /**
   * Use this method if you want to customize the style and content of the rate dialog.<br/> When
   * using the {@link AlertDialog.Builder} you should use: <ul> <li>{@link
   * AlertDialog.Builder#setPositiveButton} for the <b>rate</b> button.</li> <li>{@link
   * AlertDialog.Builder#setNeutralButton} for the <b>rate later</b> button.</li> <li>{@link
   * AlertDialog.Builder#setNegativeButton} for the <b>never rate</b> button.</li> </ul>
   *
  ...
  public void init() {
    ...
    if (launch_count >= minLaunchesUntilPrompt) {
      if (System.currentTimeMillis() >= date_firstLaunch + (minDaysUntilPrompt
  ...
  private void showDefaultDialog() {
    ...
    String message = "If you enjoy using "
        + getApplicationName(hostActivity.getApplicationContext())
  ...
  public void onClick(DialogInterface dialog, int which) {
    ...
    switch (which) {
      case DialogInterface.BUTTON_POSITIVE:
        try {
          hostActivity.startActivity(new Intent(Intent.ACTION_VIEW,
              Uri.parse("market://details?id=" + hostActivity.getPackageName())));
